...
goog.require('Blockly.PXTBlockly.Extensions'); // CASS_ADDED_PXT
...
// CASS_PXT_ADDED
Blockly.Blocks['controls_if_ext'] = {
  /**
  ...
  init: function() {
    ...
    this.appendValueInput('IF0')
        .setCheck('Boolean')
        .appendField(Blockly.Msg.CONTROLS_IF_MSG_IF_EXT_PREFIX); // CASS
    //this.appendDummyInput()
    //    .appendField(Blockly.Msg.CONTROLS_IF_MSG_IF_EXT_SUFFIX); // CASS
    this.appendDummyInput('THEN0')
        //.appendField(Blockly.Msg.CONTROLS_IF_MSG_IF_EXT_SUFFIX) // CASS
        .appendField(Blockly.Msg.CONTROLS_IF_MSG_THEN_EXT);
    ...
    this.setStyle('logic_blocks'); // CASS // https://developers.google.com/blockly/reference/js/Blockly.Block?hl=en#setStyle
  ...
  storeConnections_: function(arg) {
    ...
    if (this.getInput('DO_ELSE')) this.elseStatementConnection_ = this.getInput('DO_ELSE').connection.targetConnection;
  },
  /**
  ...
  restoreConnections_: function() {
    ...
    if (this.getInput('DO_ELSE')) Blockly.Mutator.reconnect(this.elseStatementConnection_, this, 'DO_ELSE');
  },
  addElse_: function() {
  ...
  updateShape_: function() {
    ...
    for (var i = 1; i <= this.elseifCount_; i++) {
      ...
      this.appendValueInput('IF' + i)
          .setCheck('Boolean')
          .appendField(Blockly.Msg.CONTROLS_IF_MSG_ELSEIF_EXT_PREFIX); // CASS
      //this.appendDummyInput()
      //    .appendField(Blockly.Msg.CONTROLS_IF_MSG_ELSEIF_EXT_SUFFIX); // CASS
          //.appendField("is");
      ...
      this.appendDummyInput('IFBUTTONS' + i)
          .appendField(
              new Blockly.FieldImage(this.REMOVE_IMAGE_DATAURI, 16, 16, "*", removeElseIf, false)) // 24
          .setAlign(Blockly.ALIGN_RIGHT);
      this.appendStatementInput('DO' + i);
    }
    if (this.elseCount_) {
      this.appendDummyInput('ELSETITLE')
          .appendField(Blockly.Msg.CONTROLS_IF_MSG_ELSE_EXT);
      this.appendDummyInput('ELSEBUTTONS')
          .setAlign(Blockly.ALIGN_RIGHT)
          .appendField(
              new Blockly.FieldImage(this.REMOVE_IMAGE_DATAURI, 16, 16, "*", that.removeElse_.bind(that), false)); // 24
    ...
    this.appendDummyInput('ADDBUTTON')
        .appendField(
            //new Blockly.FieldImage(this.ADD_IMAGE_DATAURI, 16, 16, "*", addElseIf, false)); // 24
            new Blockly.FieldImage(this.ADD_IMAGE_DATAURI, 16, 16, "*", addElseIf, false), null, 0, -2.5); // 24 // CASS_20220518
  },
  /**
  ...
  rebuildShape_: function() {
    ...
    if (this.getInput('DO_ELSE')) {
      elseStatementConnection = this.getInput('DO_ELSE').connection.targetConnection;
    }
    var i = 1;
    while (this.getInput('IF' + i)) {
  ...
  reconnectChildBlocks_: function(valueConnections, statementConnections,
    ...
    Blockly.Mutator.reconnect(elseStatementConnection, this, 'DO_ELSE');
  }
};
...
Blockly.defineBlocksWithJsonArray([  // BEGIN JSON EXTRACT
  // Block for boolean data type: true and false.
  {
    ...
    "output": "Boolean",
    //"outputShape": Blockly.OUTPUT_SHAPE_HEXAGONAL, // CASS_PXT_ADDED // COMMENTED
    "style": "logic_blocks",
  ...
  // CASS_PXT_COMMENTED // UNCOMMENTED
  ...
  // Block for null data type. // CASS_MODIFIED // moved to variables.js
  //{
  //  "type": "logic_null",
...
Blockly.Constants.Logic.CONTROLS_IF_MUTATOR_MIXIN = {
    ...
    while (clauseBlock) {
      switch (clauseBlock.type) {
        case 'controls_if_elseif':
          ...
          break;
        case 'controls_if_else':
          //var inputDo = this.getInput('ELSE'); // COMMENTED // CASS_BLOCKLY_MODIFIED
          var inputDo = this.getInput('ELSE'); // ADDED // CASS_BLOCKLY_MODIFIED

          // CASS_COMMENTED! // 20211101
          //clauseBlock.statementConnection_ =
          //    inputDo && inputDo.connection.targetConnection;
          break;
        default:
  ...
  rebuildShape_: function() {
    ...
    if (this.getInput('ELSE')) {
      //elseStatementConnection = this.getInput('ELSE').connection.targetConnection; // COMMENTED // CASS_BLOCKLY_MODIFIED
      elseStatementConnection = this.getInput('ELSE').connection.targetConnection; // ADDED // CASS_BLOCKLY_MODIFIED
    }
    var i = 1;
    while (this.getInput('IF' + i)) {
      var inputIf = this.getInput('IF' + i);
  ...
  updateShape_: function() {
    ...
    if (this.getInput('ELSE')) {
      ...
      this.removeInput('DO_ELSE'); // ADDED // CASS_BLOCKLY_MODIFIED
    }
    var i = 1;
    while (this.getInput('IF' + i)) {
      this.removeInput('IF' + i);
    ...
    if (this.elseCount_) {
      //this.appendStatementInput('ELSE') // FIXED // COMMENTED // CASS_BLOCKLY_MODIFIED
      this.appendDummyInput('ELSE') // ADDED
  ...
  reconnectChildBlocks_: function(valueConnections, statementConnections,
    ...
    //Blockly.Mutator.reconnect(elseStatementConnection, this, 'ELSE'); // COMMENTED // CASS_BLOCKLY_MODIFIED
    Blockly.Mutator.reconnect(elseStatementConnection, this, 'DO_ELSE'); // ADDED // CASS_BLOCKLY_MODIFIED
  }
};
