...
goog.require('Blockly.Scrollbar'); // CASS_ADDED
...
Blockly.Flyout.prototype.init = function(targetWorkspace) {
  ...
  // CASS_ADDED // LEGACY // COMMENTED
  /**
    * @type {!Blockly.Scrollbar}
    * @package
    */
  //this.scrollbar = new Blockly.Scrollbar(this.workspace_,
  //    this.horizontalLayout, false, 'blocklyFlyoutScrollbar'); // ORG
  this.scrollbar = this.workspace_.scrollbar; // CASS
...
Blockly.Flyout.prototype.updateDisplay_ = function() {
  ...
  // flyout's visibility.
  this.workspace_.scrollbar.setContainerVisible(show); // ORG
  //this.scrollbar.setContainerVisible(show); // CASS_ADDED // LEGACY
};
...
Blockly.Flyout.prototype.positionAt_ = function(width, height, x, y) {
  ...
  this.workspace_.setCachedParentSvgSize(width, height); // ORG // CASS_COMMENTED
...
// CASS_2021! // CASS_EXAMPLE
//let workspace = acs.module.blockly.workspace.current;
//workspace.toolbox_.flyout_.show(workspace.toolbox_.contents_[20].flyoutItems_);
//workspace.toolbox_.flyout_.show(workspace.toolbox_.contentMap_["blocklyTreeRow_logic"].flyoutItems_);
//workspace.toolbox_.flyout_.scrollbar.set(67);

// NEW WAY // 20210605
//console.error(acs.module.blockly.workspace.current.toolbox_.flyout_.scrollbar);
//acs.module.blockly.workspace.current.toolbox_.flyout_.scrollbar.vScroll.set(100);
 ...
 */
Blockly.Flyout.prototype.show = function(flyoutDef) { // CASS_CHECK // flyoutDef: Blockly.ToolboxCategory.flyoutItems_ // flyoutDef: workspace.toolbox_.contents_[20].flyoutItems_
 ...
 * @private
 */
Blockly.Flyout.prototype.isFlyoutQuickLocationPut = false; // CASS
Blockly.Flyout.prototype.isFlyoutQuickLocationMulti = false; // CASS
Blockly.Flyout.prototype.createFlyoutInfo_ = function(parsedContent) { // CASS_CHECK // -> Blockly.VerticalFlyout.prototype.layout_ in flyout_vertical.js


    // CASS_ADDED
    Blockly.Flyout.prototype.isFlyoutQuickLocationPut = false;
    Blockly.Flyout.prototype.isFlyoutQuickLocationMulti = false;
    if (parsedContent[0] != undefined && parsedContent[1] != undefined) {
        if (parsedContent[0].kind == 'BUTTON' && parsedContent[1].kind == 'BUTTON') {
            if (parsedContent[0]['web-class'] == "cls_block_flyout_quick_location" && parsedContent[1]['web-class'] == "cls_block_flyout_quick_location") {
                Blockly.Flyout.prototype.isFlyoutQuickLocationMulti = true;
            }
        }
    }
    ...
    switch (contentInfo['kind'].toUpperCase()) {
      ...
      case 'BUTTON':
...

        {
            ...
            // CASS_ADDED
            //if (buttonInfo["web-class"] != "cls_block_flyout_quick_location") {
            //    if (Blockly.Flyout.prototype.isFlyoutQuickLocationPut) {
            //        gaps.push(defaultGap - 8);
            //        Blockly.Flyout.prototype.isFlyoutQuickLocationPut = false;
            //    }
            //}

            contents.push({ type: 'button', button: button }); // ORG

            // CASS_ADDED // <- Blockly.FlyoutButton.prototype.createDom
            if (buttonInfo["web-class"] == "cls_block_flyout_quick_location") {

                if (Blockly.Flyout.prototype.isFlyoutQuickLocationPut == false) {

                    if (Blockly.Flyout.prototype.isFlyoutQuickLocationMulti == true) {
                        gaps.push(8);
                        Blockly.Flyout.prototype.isFlyoutQuickLocationPut = true;
                    }
                    else {
                        gaps.push(defaultGap);
                    }
