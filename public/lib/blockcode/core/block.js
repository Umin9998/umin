...
Blockly.Block = function(workspace, prototypeName, opt_id) {
  ...
  this.collapsed_ = false;
    ...
    this.collapsable_ = true; // ADDED // CASS_BLOCKLY_MODIFIED

    this.hatAppliable_ = true; // ADDED // CASS_BLOCKLY_MODIFIED

    this.external_ = true; // ADDED // CASS_BLOCKLY_MODIFIED

    this.isMenuAvailable_ = true; // ADDED // CASS_BLOCKLY_MODIFIED


    this.duplicateAvailable_ = true; // ADDED // CASS_BLOCKLY_MODIFIED

    this.duplicateLinkedAvailable_ = true; // ADDED // CASS_BLOCKLY_MODIFIED

    this.copyToPasteAvailable_ = true; // ADDED // CASS_BLOCKLY_MODIFIED

    this.copyToPasteLinkedAvailable_ = true; // ADDED // CASS_BLOCKLY_MODIFIED

    this.commentAvailable_ = true; // ADDED // CASS_BLOCKLY_MODIFIED

    this.helpAvailable_ = true; // ADDED // CASS_BLOCKLY_MODIFIED

    this.tooltipBlocked_ = false; // ADDED // CASS_BLOCKLY_MODIFIED
...
Blockly.Block.prototype.getMatchingConnection = function(otherBlock, conn) {
  ...
  if (connections.length != otherConnections.length) {
    //throw Error("Connection lists did not match in length."); // CASS_COMMENTED
  }

    if (otherConnections.length <= connections.length) {
...
Blockly.Block.prototype.setCollapsed = function(collapsed) {
  ...
  if (this.collapsed_ != collapsed) {
    ...
    ////////////////////////////////////////////////////////////////////////
    //this.setCollapsedOnce(collapsed);
...
// CASS_ADDEED
Blockly.Block.prototype.setCollapsedOnce = function (collapsed) {

    // CASS_ADDED
    // refer to // -> Blockly.Events.Delete in block_events.js
    if (collapsed == true) {
      //Blockly.Events.fire(new Blockly.Events.BlockDelete(this));
      //var _xml = Blockly.Xml.blockToDomWithXY(this);
      //console.error(_xml);
      //_xml = Blockly.Trashcan.prototype.cleanBlockXML_(_xml); // removes the id and position

        Blockly.Events.fire(new Blockly.Events.BlockUpdate(this, 'collapse'));
    }
    else {
        //this.dispose(false);
        Blockly.Events.fire(new Blockly.Events.BlockUpdate(this, 'uncollapse'));
    }

  //P
  //if (this.collapsed_ != collapsed) {
  //  //setTimeout(this.fireEventBlockChangeForSetCollapsed, 2000); // CASS_ADDED
  //  Blockly.Events.fire(new Blockly.Events.BlockChange(
  //      this, 'collapsed', null, this.collapsed_, collapsed));
  //  this.collapsed_ = collapsed;
  //}
...
// CASS_ADDED // COMMENTED
//Blockly.Block.prototype.fireEventBlockChangeForSetCollapsed = function () {
//    Blockly.Events.fire(new Blockly.Events.BlockChange(
//    this, 'collapsed', null, !this.collapsed_, this.collapsed_));
//}
 ...
 */
Blockly.Block.prototype.toString = function(opt_maxLength, opt_emptyToken) { // CASS // MODIFIED // CHECK_2021
  var text = [];
  //var emptyFieldPlaceholder = opt_emptyToken || '?'; // ORG
  var emptyFieldPlaceholder = opt_emptyToken || '💬'; // CASS_ADDED
  ...
  //text = text.join(' ').replace(/(\() | (\))/gmi, '$1$2').trim() || '???'; // ORG
  text = text.join(' ').replace(/(\() | (\))/gmi, '$1$2').trim() || '💬'; // CASS_ADDED
  if (opt_maxLength) {
...
Blockly.Block.prototype.removeInput = function(name, opt_quiet) {
  ...
  if (opt_quiet) {
    ...
    return false; // CASS_ADDED
    //throw Error('Input not found: ' + name); // ORG
  }
};
