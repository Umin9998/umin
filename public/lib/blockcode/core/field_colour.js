 ...
 * @constructor
 */
//Blockly.FieldColour = function(opt_value, opt_validator, opt_config) { // ORG
Blockly.FieldColour = function (opt_value, opt_validator, opt_config, width, height) { // CASS_BLOCKLY_MODIFIED
  ...
  this.onKeyDownWrapper_ = null;
    ...
    if (width !== undefined && height !== undefined) {
        ...
        if (!isNaN(width)) {
            ...
            _width = Blockly.FieldColour.DEFAULT_WIDTH;
            //_width = this.getConstants().FIELD_COLOUR_DEFAULT_WIDTH;
        }
        if (!isNaN(height)) {
            _height = parseInt(height);
        }
        else {
            _height = Blockly.FieldColour.DEFAULT_HEIGHT;
            //_height = this.getConstants().FIELD_COLOUR_DEFAULT_HEIGHT;
        }
        this.size_ = new Blockly.utils.Size(_width, _height);
...
Blockly.FieldColour.prototype.dropdownCreate_ = function() {
  ...
  for (var i = 0; i < colours.length; i++) {
    ...
    var cell = document.createElement('td');
      ...
      var colorText = "black"; // ADDED // CASS_BLOCKLY_MODIFIED
      if (parseInt(i % 7) > 2) colorText = "white"; // ADDED // CASS_BLOCKLY_MODIFIED
      cell.innerHTML = "<text style='font-size:12px;padding-left:5px;pointer-events:none;color:" + colorText + ";'>" + String.fromCharCode(parseInt(i / 7) + 65) + (parseInt(i % 7) + 1).toString() + "</text>"; // ADDED // CASS_BLOCKLY_MODIFIED
