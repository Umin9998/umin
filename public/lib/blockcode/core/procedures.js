...
Blockly.Procedures.allProcedures = function (root) {
  ...
  //return [proceduresNoReturn, proceduresReturn];
    ...
    var proceed = true;
    if (typeof acs === 'undefined') proceed = false;
    else if (typeof acs.module === 'undefined') proceed = false;
    else if (typeof acs.module.blockly === 'undefined') proceed = false;
    else if (typeof acs.module.blockly.project === 'undefined') proceed = false;
    else if (typeof acs.module.blockly.project.workspaces === 'undefined') proceed = false;
    if (proceed === true && acs.module.blockly.project.workspaces.length > 1) {
        for (var w = 0; w < acs.module.blockly.project.workspaces.length; w++) {

            var blocks = acs.module.blockly.project.workspaces[w].getAllBlocks(false);
...
Blockly.Procedures.flyoutCategory = function(workspace) {
  ...
  //}
    ...
    // CASS_BLOCKLY_MODIFIED // added
    if (Blockly.Blocks['procedures_return_with_value']) {
        // <block type="procedures_return" gap="16"></block>
        var block = Blockly.utils.xml.createElement('block');
        block.setAttribute('type', 'procedures_return_with_value');
        block.setAttribute('gap', 16);
        xmlList.push(block);
    }
...
Blockly.Procedures.getDefinition = function (name, workspace) {
    ...
    else if (typeof acs.module.blockcode === 'undefined') proceed = false;
    else if (typeof acs.module.blockcode.project === 'undefined') proceed = false;
    else if (typeof acs.module.blockcode.project.workspaces === 'undefined') proceed = false;
    if (proceed === true && acs.module.blockcode.project.workspaces.length > 1) {
        for (var w = 0; w < acs.module.blockcode.project.workspaces.length; w++) {

            var blocks = acs.module.blockcode.project.workspaces[w].getAllBlocks(false);

            for (var i = 0; i < blocks.length; i++) {
                console.error("blocks[" + i + "].getProcedureDef");
                console.error(blocks[i].getProcedureDef);
