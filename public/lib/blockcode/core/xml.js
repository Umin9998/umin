  ...
  try {
    for (var i = 0, xmlChild; (xmlChild = xml.childNodes[i]); i++) {
      ...
      if (name == 'block' ||
        ...
        continue; // CASS_2021!! // CASS_SHADOW
        //throw TypeError('Shadow block cannot be a top-level block.'); // ORG
 ...
 * @private
 */
Blockly.Xml.domToBlockHeadless_ = function(xmlBlock, workspace, // CASS // Check old solution
...
// there was an old solution for Blockly.Xml.domToBlockHeadless_ like the following
//if (!input.connection) { // CASS_2021_ADDED // refer to old ref // it might be an old legacy solution
//    break;
//}