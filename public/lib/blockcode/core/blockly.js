...
Blockly.clipboardXmlBackup_ = null; // ADDED // CASS_BLOCKLY_MODIFIED
...
Blockly.clipboardSourceBackup_ = null; // ADDED // CASS_BLOCKLY_MODIFIED
...
// CASS_ADDED
Blockly.copy = function (toCopy, withLinked) { // withLinked ADDED // CASS_BLOCKLY_MODIFIED
    var data = toCopy.toCopyData(withLinked); // withLinked ADDED // CASS_BLOCKLY_MODIFIED // -> block_svg.js
    if (data) {
        Blockly.clipboardXml_ = data.xml;
        Blockly.clipboardSource_ = data.source;
        Blockly.clipboardTypeCounts_ = data.typeCounts;
...
Blockly.paste = function() {
  ...
  //}
    ...
    // CASS_ADDED
    if (typeof acs_workspace_current !== 'undefined' && acs_workspace_current !== null) { // ADDED // CASS_BLOCKLY_MODIFIED
        workspace = acs_workspace_current;
    }
...
// refer generateContextMenu in block_svg.js
// ADDED // CASS_BLOCKLY_MODIFIED // -> Blockly.ContextMenuItems.registerDuplicateLinked in contextmenu_items.js
Blockly.duplicateLinked = function (toDuplicate) {
    // Save the clipboard.
    var clipboardXml = Blockly.clipboardXml_;
    var clipboardSource = Blockly.clipboardSource_;

    // Create a duplicate via a copy/paste operation.
    Blockly.copy(toDuplicate, true); // <- Blockly.copy in here <- toCopyData in block_svg.js
    toDuplicate.workspace.paste(Blockly.clipboardXml_);
...
// ADDED // CASS_BLOCKLY_MODIFIED // -> Blockly.ContextMenuItems.registerCopyToPaste in contextmenu_items.js
Blockly.copyToPaste = function (toDuplicate, linked) {
    // Save the clipboard.
    Blockly.clipboardXmlBackup_ = Blockly.clipboardXml_;
    Blockly.clipboardSourceBackup_ = Blockly.clipboardSource_;

    // Create a duplicate via a copy/paste operation.
    if (linked === true) {
        Blockly.copy(toDuplicate, linked); // <- Blockly.copy in here <- toCopyData in block_svg.js
    }
    else {
        Blockly.copy(toDuplicate);
    }
