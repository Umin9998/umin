...
Blockly.Variables.flyoutCategoryBlocks = function(workspace) {
  ...
  var xmlList = [];
    ...
    if (typeof acs_common_config !== 'undefined') {
        if (acs_common_config.projectLevel > 2) {
            if (Blockly.Blocks['variables_value_undefined']) {
                var block = Blockly.utils.xml.createElement('block');
                block.setAttribute('type', 'variables_value_undefined');
                xmlList.push(block);
            }
        }
    }
  ...
  if (variableModelList.length > 0) {
    ...
    if (Blockly.Blocks['variables_set']) {
      ...
      // CASS_BLOCKLY_MODIFIED // added
      if (Blockly.Blocks['variables_set_local']) {
          var block = Blockly.utils.xml.createElement('block');
          block.setAttribute('type', 'variables_set_local');
          //block.setAttribute('gap', Blockly.Blocks['math_change'] ? 8 : 24);
          block.appendChild(
              Blockly.Variables.generateVariableFieldDom(mostRecentVariable));
          xmlList.push(block);
