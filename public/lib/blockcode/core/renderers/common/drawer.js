...
Blockly.blockRendering.Drawer.prototype.draw = function() {
  ...
  this.drawOutline_();
  this.drawInternals_(); // CASS_2021!! // tranform is set here!!
 ...
 * @protected
 */
Blockly.blockRendering.Drawer.prototype.drawOutline_ = function() { // CASS_2021! <- Blockly.blockRendering.Renderer.prototype.render in renderer.js
  this.drawTop_();
  ...
  // CASS_2021!
  // CASS_ADDED // for procedures_callreturn shape // CASS_PROCEDURES
  //console.error(this.block_);
  if (this.block_.callType_ == "procedures_callreturn") { // if (this.block_.type == "procedures_defreturn")
    this.outlinePath_ = this.outlinePath_.replace("v 22 a 3", "v 21 a 3");
  }

  // CASS_ADDED // for controls_if shape // CASS_CONTROLS_IF
 ...
 * @protected
 */
Blockly.blockRendering.Drawer.prototype.drawTop_ = function() { // CASS_CHECK
  var topRow = this.info_.topRow;
  var elements = topRow.elements;

  this.positionPreviousConnection_();
  this.outlinePath_ +=
      Blockly.utils.svgPaths.moveBy(topRow.xPos, this.info_.startY); // CASS_CHECK // m 0,0
  for (var i = 0, elem; (elem = elements[i]); i++) {
    if (Blockly.blockRendering.Types.isLeftRoundedCorner(elem)) {
      ...
      this.outlinePath_ += Blockly.utils.svgPaths.lineOnAxis('h', elem.width); // CASS_CHECK // m 0,0 h 160
    }
...
Blockly.blockRendering.Drawer.prototype.drawJaggedEdge_ = function(row) {
  ...
  //this.outlinePath_ = this.outlinePath_.replace("v 14  H", "v 0  H"); // ADDED FOR HAT // CASS_BLOCKLY_MODIFIED // CASS_CHECK
};
...
Blockly.blockRendering.Drawer.prototype.drawValueInput_ = function(row) {
  ...
  this.positionExternalValueConnection_(row); // CASS_2021! // CASS_INPUT
 ...
 * @protected
 */
Blockly.blockRendering.Drawer.prototype.drawLeft_ = function() { // CASS // <- Blockly.blockRendering.Drawer.prototype.drawOutline_ <- Blockly.blockRendering.Drawer.prototype.draw
  ...
  this.outlinePath_ += 'z'; // ORG
  // CASS_BLOCKLY_MODIFIED // CASS_CHECK
  // COMMENTED
  //if (!outputConnection) {
  //  this.outlinePath_ += 'z';
  //}
...
Blockly.blockRendering.Drawer.prototype.layoutField_ = function(fieldInfo) {
  if (Blockly.blockRendering.Types.isField(fieldInfo)) {
    ...
    if (fieldInfo.field != undefined && fieldInfo.field != null) {
        _x_offset = fieldInfo.field.x_offset;
        _y_offset = fieldInfo.field.y_offset;
    }

  //var yPos = fieldInfo.centerline - fieldInfo.height / 2; // ORG
  //var xPos = fieldInfo.xPos; // ORG
  var yPos = fieldInfo.centerline - fieldInfo.height / 2 + _y_offset; // CASS_2021!! // transform
  var xPos = fieldInfo.xPos + _x_offset; // CASS_2021!! // transform
  var scale = '';
  if (this.info_.RTL) {
 ...
 * @protected
 */
Blockly.blockRendering.Drawer.prototype.drawInlineInput_ = function(input) { // CASS_2021! // CASS_INPUT
...
Blockly.blockRendering.Drawer.prototype.positionInlineInputConnection_ = function(input) {
  var yPos = input.centerline - input.height / 2; // ORG
  //var yPos = input.height / 2 - input.connectionHeight; // CASS_ADDED // COMMENTED
  // Move the connection.
 ...
 * @protected
 */
Blockly.blockRendering.Drawer.prototype.positionExternalValueConnection_ = function(row) { // CASS_2021! // CASS_INPUT
  var input = row.getLastInput();
  if (input.connectionModel) {
    ...
    if (this.info_.RTL) {
      ...
      // CASS // CASS_PUZZLE_TAB // COMMENTED
      //if (input.connectedBlock !== undefined && input.connectedBlock !== null) {
      //    var _blockHeight = input.connectedBlock.outputConnection.sourceBlock_.height; // <- refer - Blockly.blockRendering.OutputConnection in connections.js
      //    var _connectionOffsetY = _blockHeight / 2 - input.connectionHeight / 2;

      //    input.connectionModel.setOffsetInBlock(connX, _connectionOffsetY); // CASS_ADDED_MODIFIED
      //}
      //else {
      //    input.connectionModel.setOffsetInBlock(connX, row.yPos);
      //}
