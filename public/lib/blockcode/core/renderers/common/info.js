...
Blockly.blockRendering.RenderInfo = function(renderer, block) {
  ...
  this.outputConnection = !block.outputConnection ? null :
      new Blockly.blockRendering.OutputConnection( // CASS <- core/renderers/measureables/connections.js
          this.constants_,
 ...
 * @package
 */
Blockly.blockRendering.RenderInfo.prototype.measure = function() { // CASS_2021! // CHECK for drawing // <- Blockly.blockRendering.Renderer.prototype.render in renderer.js
  this.createRows_();
  this.addElemSpacing_();
  this.addRowSpacing_();
  this.computeBounds_(); // CASS // CEHCK
...
Blockly.blockRendering.RenderInfo.prototype.populateTopRow_ = function() {
  ...
  var leftSquareCorner = this.topRow.hasLeftSquareCorner(this.block_); // in core\measureables\rows.js

  // ADDED // CASS_BLOCKLY_MODIFIED
  // COMMENTED
  //if (Blockly.BlockSvg.START_HAT && this.block_.hatAppliable_ === false) { // in core\block.js // this.hatAppliable_ = true;
  //  leftSquareCorner = false;
  //}
  ...
  if (hasHat) {
    ...
    // CASS_ADDED
    var hat;
    if (this.block_.hatAppliable_ === true) { // ADDED // CASS_BLOCKLY_MODIFIED // -> block.js
      hat = new Blockly.blockRendering.Hat(this.constants_);
      this.topRow.elements.push(hat);
      this.topRow.capline = hat.ascenderHeight;
    }
  } else if (hasPrevious) {
 ...
 */
// TODO: More cleanup.
Blockly.blockRendering.RenderInfo.prototype.computeBounds_ = function() { // THIS IS IT! // CASS_CHECK
  ...
  for (var i = 0, row; (row = this.rows[i]); i++) {
    ...
    //if (row instanceof Blockly.blockRendering.InputRow) { // -> Blockly.blockRendering.InputRow in core\renderers\measurables\rows.js
    //    //console.error(row);
    //}

    row.measure(); // -> Blockly.blockRendering.InputRow.prototype.measure in core\renderers\measurables\rows.js
