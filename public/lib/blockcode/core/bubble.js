...
Blockly.Bubble = function(
  ...
  this.shape_ = shape;
    ...
    // CASS
    this.bubbleType_ = content.attributes[0].nodeValue;

    // CASS
    this.BORDER_WIDTH_BUBBLE = 10;
    if (this.bubbleType_ == "blocklyCommentBubble") {
        this.BORDER_WIDTH_BUBBLE = 1;
    }
 ...
 */
//Blockly.Bubble.BORDER_WIDTH = 6; // ORG
Blockly.Bubble.BORDER_WIDTH = 1; //6; // CASS
Blockly.Bubble.BORDER_WIDTH_MUTATOR = 10; // CASS_ADDED
...
Blockly.Bubble.RESIZE_SIZE = 12;

/**
 * Determines the thickness of the base of the arrow in relation to the size
 * of the bubble.  Higher numbers result in thinner arrows.
 */
//Blockly.Bubble.ARROW_THICKNESS = 5; // ORG
Blockly.Bubble.ARROW_THICKNESS = 100; //5; // CASS

/**
 * The number of degrees that the arrow bends counter-clockwise.
 */
//Blockly.Bubble.ARROW_ANGLE = 20; // ORG
Blockly.Bubble.ARROW_ANGLE = 0; //20; // CASS

/**
 * The sharpness of the arrow's bend.  Higher numbers result in smoother arrows.
 */
//Blockly.Bubble.ARROW_BEND = 4; // ORG
Blockly.Bubble.ARROW_BEND = 1000; // 4; // CASS

/**
 * Distance between arrow point and anchor point.
 */
//Blockly.Bubble.ANCHOR_RADIUS = 8; // ORG
Blockly.Bubble.ANCHOR_RADIUS = 0; // 8; // CASS
 ...
 * @private
 */
Blockly.Bubble.prototype.useArrow_ = true;
...
Blockly.Bubble.prototype.createDom_ = function(content, hasResize) {
  ...
  */
  ...
  //}
    ...
    var filter = {}; // CASS // ADDED

    if (Blockly.utils.userAgent.JAVA_FX || !this.useArrow_) { // pxt-blockly // CASS // ADDED
        //if (Blockly.utils.userAgent.JAVA_FX) { // ORG
        // Multiple reports that JavaFX can't handle filters.
        // https://github.com/google/blockly/issues/99
        filter = {};
    }
  ...
  //    Blockly.utils.Svg.PATH, {}, bubbleEmboss);
    ...
    // CASS_ADDED
    this.bubbleArrow_ = Blockly.utils.dom.createSvgElement(this.useArrow_ ? Blockly.utils.Svg.PATH : 'line', {}, // pxt-blockly
        bubbleEmboss);
  ...
  this.bubbleBack_ = Blockly.utils.dom.createSvgElement(
      Blockly.utils.Svg.RECT, {
        ...
        'x': 0,
        'y': 0,
        //'rx': Blockly.Bubble.BORDER_WIDTH, // ORG
        //'ry': Blockly.Bubble.BORDER_WIDTH  // ORG
        'rx': 3, // this.BORDER_WIDTH_BUBBLE, // CASS
        'ry': 3 // this.BORDER_WIDTH_BUBBLE  // CASS
      },
      bubbleEmboss);

  this.bubbleGroup_.appendChild(content); // pxt-blockly // CASS_PXT // ADDED
  ...
  if (hasResize) {
    ...
    //var resizeSize = 2 * Blockly.Bubble.BORDER_WIDTH; // ORG
    var resizeSize = Blockly.Bubble.RESIZE_SIZE; // CASS_PXT

    Blockly.utils.dom.createSvgElement(
        Blockly.utils.Svg.POLYGON,
        //{'points': '0,x x,x x,0'.replace(/x/g, resizeSize.toString())}, // ORG

        { 'points': '-2,x x,x x,-2'.replace(/x/g, (resizeSize - 1).toString()) }, // CASS
  ...
  //this.bubbleGroup_.appendChild(content); // ORG // CASS_PXT_COMMENTED
...
Blockly.Bubble.prototype.setBubbleSize = function (width, height) {
  ...
  //var doubleBorderWidth = 2 * Blockly.Bubble.BORDER_WIDTH; // ORG
  var doubleBorderWidth = 2 * this.BORDER_WIDTH_BUBBLE; // CASS
  ...
  if (this.resizeGroup_) {
    ...
    var resizeSize = Blockly.Bubble.RESIZE_SIZE; // ADDED // CASS_PXT

    if (this.workspace_.RTL) {
      // Mirror the resize group.
      //var resizeSize = 2 * Blockly.Bubble.BORDER_WIDTH; // ORG // CASS_COMMENTED
...
Blockly.Bubble.prototype.renderArrow_ = function () {
    // CASS
    var _comments = document.getElementsByClassName("blocklyCommentTextarea");

    for (var i = 0; i < _comments.length; i++) {

        if (_comments[i].clientHeight < 60) { // CASS_2021! // SCROLL
            _comments[i].style.overflowY = "hidden";
        }
        else {
            _comments[i].style.overflowY = "scroll";
        }
    }
 ...
 * @private
 */
//Blockly.Bubble.prototype.renderArrow_ = function() { // ORG
Blockly.Bubble.prototype.drawArrow_ = function() { // CASS_PXT
  var steps = [];
 ...
 */
//Blockly.Bubble.prototype.setColour = function (hexColour) { // ORG
Blockly.Bubble.prototype.setColour = function (hexColour, secondaryColour) { // CASS
  this.bubbleBack_.setAttribute('fill', hexColour);
  //this.bubbleArrow_.setAttribute('fill', hexColour); // ORG

    // pxt-blockly: Support line and arrow rendering // CASS_PXT // ADDED
    if (this.useArrow_) {
...
Blockly.Bubble.textToDom = function(text) {
  var paragraph = Blockly.utils.dom.createSvgElement(
      Blockly.utils.Svg.TEXT, {
        ...
        //'y': Blockly.Bubble.BORDER_WIDTH // ORG
        'y': this.BORDER_WIDTH_BUBBLE // CASS
      },
      null);
  ...
  for (var i = 0; i < lines.length; i++) {
    var tspanElement = Blockly.utils.dom.createSvgElement(
        ...
        //{'dy': '1em', 'x': Blockly.Bubble.BORDER_WIDTH}, paragraph); // ORG
        { 'dy': '1em', 'x': this.BORDER_WIDTH_BUBBLE }, paragraph); // CASS
...
Blockly.Bubble.createNonEditableBubble = function(
  ...
  if (block.RTL) {
    ...
    for (var i = 0, textElement; (textElement = paragraphElement.childNodes[i]);
      ...
      //textElement.setAttribute('x', maxWidth + Blockly.Bubble.BORDER_WIDTH); // ORG
      textElement.setAttribute('x', maxWidth + this.BORDER_WIDTH_BUBBLE); // CASS
    }
  }
  return bubble;
};
