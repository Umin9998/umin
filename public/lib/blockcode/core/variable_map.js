...
// CASS // ADDED
if (Blockly.Msg["WARNING_RESERVED_WORDS"] == undefined) Blockly.Msg["WARNING_RESERVED_WORDS"] = "CANNOT USE RESERVED WORDS AS VARIABLE NAMES!";
...
Blockly.VariableMap.prototype.checkReservedWord = function (name) {
    ...
    if (
        ...
        if (typeof acs_function_alert_cannotUseReservedWords !== 'undefined') {
            acs_function_alert_cannotUseReservedWords();
        }
        else {
            alert(Blockly.Msg["WARNING_RESERVED_WORDS"]); // korean makes an error when building Blockly
            //throw Error(Blockly.Msg["WARNING_RESERVED_WORDS"]);
        }
        return true;
    }
}
...
Blockly.VariableMap.prototype.renameVariableAndUses_ = function(variable,
    ...
    // CASS_ADDED
    var _result = this.checkReservedWord(newName);
    if (_result) return;
...
Blockly.VariableMap.prototype.createVariable = function(name,
  ...
  if (variable) {
    ...
    // CASS_ADDED
    var _result = this.checkReservedWord(name);
    if (_result) return;
